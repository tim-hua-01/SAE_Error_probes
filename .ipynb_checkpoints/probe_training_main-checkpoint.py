    ...
    for i in tqdm(range(0, n, batch_size), desc="Generating features"):
        ...
        batch_out = model.run_with_cache_with_saes(
            ...
            names_filter=lambda name: name in [
                'blocks.21.hook_resid_post.hook_sae_input',
                'blocks.21.hook_resid_post.hook_sae_recons'
            ]
        )[1]
        act_input = extract_last_token_acts(batch_out['blocks.21.hook_resid_post.hook_sae_input'], batch_mask, offset)
        act_recons = extract_last_token_acts(batch_out['blocks.21.hook_resid_post.hook_sae_recons'], batch_mask, offset)
...
def record_active_latents(tokenized, model, sae, batch_size=8, device='cuda', offset=1):
    ...
    for i in tqdm(range(0, n, batch_size), desc="Recording active latents"):
        ...
        batch_out = model.run_with_cache_with_saes(
            batch_ids,
            saes=sae,
            names_filter=lambda name: name in ['blocks.21.hook_resid_post.hook_sae_acts_post']
        )[1]
        
        # Extract specified token activations
        acts = extract_last_token_acts(
            batch_out['blocks.21.hook_resid_post.hook_sae_acts_post'], 
            batch_mask,
            offset
        )
    ...
    for i in tqdm(range(0, n, batch_size), desc="Computing residual-probe dot products"):
        ...
        with t.no_grad():
            cache = model.run_with_cache(
                batch_ids, 
                names_filter=lambda x: x == 'blocks.21.hook_resid_post'
            )[1]
            residual_activations = cache['blocks.21.hook_resid_post']
